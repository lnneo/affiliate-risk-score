...
import { ShieldCheck, RefreshCw, ShoppingBag, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function RealReferralLandingPage({ params }: { params: Promise<{ affiliateId: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#090d16] text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-16 text-center space-y-8 flex flex-col items-center justify-center">
        {loading ? (
          <div className="glass-card p-12 rounded-3xl border border-slate-800 space-y-4 max-w-md w-full">
            ...
            <h2 className="text-xl font-bold text-slate-100">Đang Thu thập Fingerprint Thật...</h2>
            <p className="text-xs text-slate-400">
              FingerprintJS đang đo đạc thông số phần cứng trình duyệt thực tế của thiết bị này.
            </p>
          </div>
        ) : (
          <div className="glass-card p-10 rounded-3xl border border-emerald-500/30 bg-emerald-950/10 space-y-6 max-w-xl w-full animate-fade-in">
            <div className="h-16 w-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 mx-auto glow-emerald">
              <CheckCircle2 className="h-8 w-8" />
            </div>

            <div className="space-y-2">
              <h2 className="text-2xl font-extrabold text-slate-100">Đã Ghi Nhận Lượt Nhấp Link Thành Công!</h2>
              <p className="text-xs text-slate-400">
            ...
            <div className="bg-slate-950/80 p-4 rounded-2xl border border-slate-800 text-xs font-mono text-left space-y-2">
              <div className="text-slate-400 font-sans font-bold border-b border-slate-800 pb-2 flex justify-between">
                <span>Thông số Thiết bị Thực tế thu thập:</span>
                <span className="text-emerald-400">Ref: {affiliateId}</span>
              </div>
              <div className="text-slate-300">Fingerprint Hash Thật: <span className="text-indigo-400 font-bold">{realFingerprint}</span></div>
              <div className="text-slate-300">Cookie ID: <span className="text-slate-200">{clickResult?.cookieId}</span></div>
              <div className="text-slate-300">IP Thiết bị: <span className="text-slate-200">{clickResult?.clientIp || 'Lấy từ IP Kết nối'}</span></div>
            </div>
            ...
            <div className="pt-2">
              ...
              >
                Chuyển đến Cửa hàng Mua hàng Ngay <ArrowRight className="h-4 w-4" />
              </button>
            </div>
          </div>
        )}
