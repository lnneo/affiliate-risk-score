...
import { Sliders, RefreshCw, Power, Ban, Plus, Trash2, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#090d16] text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-8 sm:px-6 space-y-8">
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
            <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <span className="font-bold text-emerald-400 block">DUYỆT (APPROVE)</span>
              <span className="text-slate-400 text-[11px]">Tổng điểm &lt; 40 điểm</span>
            </div>
            <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
              <span className="font-bold text-blue-400 block">TẠM GIỮ (PENDING)</span>
              <span className="text-slate-400 text-[11px]">Tổng điểm 40 – 69 điểm</span>
            </div>
            <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
              <span className="font-bold text-amber-400 block">KIỂM TRA THỦ CÔNG</span>
              <span className="text-slate-400 text-[11px]">Tổng điểm 70 – 99 điểm</span>
            </div>
            <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20">
              <span className="font-bold text-rose-400 block">TỪ CHỐI (REJECT)</span>
        ...
        <div className="glass-card p-6 rounded-2xl border border-rose-500/30 space-y-5 bg-rose-950/10">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-sm font-bold uppercase tracking-wider text-rose-400 flex items-center gap-2">
              <Ban className="h-4 w-4" /> Quản lý Danh Sách Đen Bảo Mật (Attribute Blacklist)
            </h3>
            <span className="text-[11px] text-slate-400 font-mono">Đã thêm: {blacklists.length} mục</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <button
              ...
              className="py-2 px-4 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-rose-600/30 transition-all disabled:opacity-50"
            >
                ...
                {blacklists.length === 0 ? (
                  ...
                  blacklists.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-900/40">
                      ...
                      <td className="px-3 py-2 font-bold text-slate-200">{item.value}</td>
                      <td className="px-3 py-2 text-slate-400 font-sans text-xs">{item.reason || 'N/A'}</td>
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-slate-100 font-mono">{rule.rule_type}</span>
                  </div>
                ...
                <p className="text-xs text-slate-400 mb-4 min-h-[32px] leading-relaxed">{rule.description}</p>

                <div className="flex items-center justify-between gap-4 pt-2">
                  <div className="flex-1 space-y-1">
                    <label className="text-[11px] font-semibold text-slate-400 block">Trọng số Điểm Cộng (+Points)</label>
                    <input
                      type="range"
                      min="0"
                      max="150"
