...
const PRESET_SCENARIOS: Scenario[] = [
  ...
  },
  {
    ...
    form: {
      ...
      externalCustomerId: 'cust_external_9999', // Duplicate Customer ID!
      isVpn: false,
      isDatacenter: false,
      amount: 199.00,
    },
      ...
      case 'APPROVE':
        return (
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-bold text-base glow-emerald">
            ...
            <span>DUYỆT HOA HỒNG (APPROVE - Giao dịch sạch)</span>
          </div>
        );
      case 'PENDING_REVIEW':
        return (
            ...
            {PRESET_SCENARIOS.map((scen) => {
              ...
              return (
                <button
                  ...
                  className={`text-left p-4 rounded-xl border transition-all glass-card-hover ${
                    isSelected
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-sm text-slate-100">{scen.name}</span>
        ...
        <div className="glass-card p-5 rounded-2xl border border-indigo-500/30 bg-indigo-950/20 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs font-mono">
            <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
              ...
              <span className="font-bold text-slate-200">{AFFILIATE_PROMOTER.email}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
              <span className="text-[10px] text-slate-400 font-sans block mb-0.5">Tài khoản Thanh toán:</span>
              <span className="font-bold text-slate-200">{AFFILIATE_PROMOTER.paymentAccount}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
              <span className="text-[10px] text-slate-400 font-sans block mb-0.5">Vân tay Thiết bị:</span>
              <span className="font-bold text-indigo-400">{AFFILIATE_PROMOTER.deviceFingerprint}</span>
            </div>
            <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
              <span className="text-[10px] text-slate-400 font-sans block mb-0.5">IP Đăng ký:</span>
              <span className="font-bold text-slate-200">{AFFILIATE_PROMOTER.registeredIp}</span>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4 text-xs">
                <div>
                  <label className="text-slate-400 font-medium flex items-center gap-1.5 mb-1.5">
                    <Key className="h-3.5 w-3.5 text-indigo-400" /> ID Người giới thiệu (Affiliate ID)
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-slate-400 font-medium flex items-center gap-1.5">
                      <Mail className="h-3.5 w-3.5 text-indigo-400" /> Email Người mua hàng
                    </label>
                    {isEmailMatched && (
                      <span className="text-[10px] text-rose-400 font-bold bg-rose-500/10 px-1.5 py-0.5 rounded border border-rose-500/20 flex items-center gap-1">
                  ...
                  <input
                    ...
                    className={`w-full bg-slate-950/80 border rounded-lg px-3 py-2 text-slate-200 font-mono focus:outline-none ${
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-slate-400 font-medium flex items-center gap-1.5">
                      <CreditCard className="h-3.5 w-3.5 text-indigo-400" /> Tài khoản Thanh toán (PayPal / Thẻ)
                    </label>
                    {isPaymentMatched && (
                      <span className="text-[10px] text-rose-400 font-bold bg-rose-500/10 px-1.5 py-0.5 rounded border border-rose-500/20 flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="text-slate-400 font-medium flex items-center gap-1.5">
                        <Globe className="h-3.5 w-3.5 text-indigo-400" /> Địa chỉ IP
                      </label>
                      {isIpMatched && (
                  ...
                  <div>
                    <label className="text-slate-400 font-medium flex items-center gap-1.5 mb-1.5">
                      <Globe className="h-3.5 w-3.5 text-indigo-400" /> Quốc gia (Country ISO)
                    </label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-slate-400 font-medium flex items-center gap-1.5 mb-1.5">
                      <Laptop className="h-3.5 w-3.5 text-indigo-400" /> Vân tay Thiết bị
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-slate-400 font-medium flex items-center gap-1.5 mb-1.5">
                      <CopyCheck className="h-3.5 w-3.5 text-indigo-400" /> Mã Khách Hàng (Customer ID)
                    </label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-slate-400 font-medium flex items-center gap-1.5 mb-1.5">
                    <LinkIcon className="h-3.5 w-3.5 text-indigo-400" /> Trang Giới thiệu (Referrer URL)
                  </label>
                  <input
                    type="text"
              ...
              <div className="glass-card p-6 rounded-2xl border border-slate-800 space-y-6 animate-fade-in">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
                  <div className="space-y-1">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider">Kết quả Đối soát Risk Engine</span>
                ...
                <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-1.5">
                    <ArrowRightLeft className="h-4 w-4" /> Bảng Đối soát Trực quan: Người Mua vs Người Giới Thiệu
                  </h4>
                  <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                    <div className="space-y-1 bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                      ...
                      <div>Email: <span className={isEmailMatched ? 'text-rose-400 font-bold' : 'text-slate-200'}>{formData.userEmail}</span></div>
                      <div>PayPal: <span className={isPaymentMatched ? 'text-rose-400 font-bold' : 'text-slate-200'}>{formData.paymentAccount}</span></div>
                      <div>Fingerprint: <span className={isFingerprintMatched ? 'text-amber-400 font-bold' : 'text-slate-200'}>{formData.fingerprintHash}</span></div>
                      <div>IP: <span className={isIpMatched ? 'text-amber-400 font-bold' : 'text-slate-200'}>{formData.ip}</span></div>
                    </div>
                    <div className="space-y-1 bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                ...
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-2 text-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-slate-400 font-mono text-[11px]">
                    <div>Mã Đánh giá (Risk ID): <span className="text-slate-200">{evaluationResult.riskScoreId}</span></div>
                    <div>Mã Đơn hàng (Order ID): <span className="text-slate-200">{evaluationResult.orderId}</span></div>
                    <div>Affiliate ID: <span className="text-slate-200">{evaluationResult.affiliateId}</span></div>
                    <div>ID Người mua: <span className="text-slate-200">{evaluationResult.userId}</span></div>
                  </div>
                </div>
              </div>
            )}
